...
describe("RAmMMF Contract", function () {
    ...
    describe("Initialization", function () {
        it("Should initialize with correct values", async function () {
            ...
            expect(await rAmMMF.oracle()).to.equal(await oracle.getAddress());
        });
    });
    ...
    describe("Wrap Functionality", function () {
        ...
        it("Should revert if wrapping zero tokens", async function () {
            await expect(rAmMMF.connect(addr1).wrap(0)).to.be.revertedWith("zero AmMMF tokens");
        });
    });
    ...
    describe("Unwrap Functionality", function () {
        ...
        it("Should revert if unwrapping zero tokens", async function () {
            await expect(rAmMMF.connect(addr1).unwrap(0)).to.be.revertedWith("zero rAmMMF tokens");
        });
        ...
        it("Should revert if unwrapping too small amount", async function () {
            ...
            await expect(rAmMMF.connect(addr1).unwrap(amount)).to.be.revertedWith("UnwrapTooSmall");
        });
    });
    ...
    describe("Admin Functions", function () {
        ...
        it("Should revert if setting oracle to zero address", async function () {
            await expect(rAmMMF.setOracle(ethers.ZeroAddress)).to.be.revertedWith("CannotSetToZeroAddress");
        });
    ...
    describe("Transfer Functionality", function () {
        ...
        it("Should revert if transferring more than balance", async function () {
            const amount = ethers.parseUnits("100", 18);
            await expect(rAmMMF.connect(addr1).transfer(addr2.address, amount)).to.be.revertedWith("TRANSFER_AMOUNT_EXCEEDS_BALANCE");
        });
    });
});